...
skill's pre-2.1.0 name is removed from the working tree (historical
CHANGELOG entries and ADR-014 now say "its former name" / "the former
name"), the whole git history is rewritten (paths, blob contents, and
commit messages mapped to the current name — old revisions read as if
the skill was always named spec-to-prod), and the pre-rename project
memory on this machine is deleted. This entry is the record that a
redaction happened, and why; the changelog otherwise stays append-only.
...
Skill renamed from its former name → spec-to-prod — directory, router command
(router command renamed to `/spec-to-prod`), plugin manifests (regenerated:
...
root: the router command file moves from the package-root `commands/`
into the skill's own `commands/`, so the skill no longer has any
...
the plugin-prefixed `:_shared-protocol` warning in Claude Code's plugin agent
...
omp-defs.py) moves from the skill's `tools/` to the repo root,
...
the zcode skill root as a valid path — sync.sh had simply never
...
the omp-native skill copy under ~/.omp/agent/skills/
...
the router command's `scaffold` verb updated.
...
pass rejected everything the skill already had:
