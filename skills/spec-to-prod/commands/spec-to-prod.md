...
argument-hint: <scaffold|bugfix|survey|research|plan|tech|tasks|qa|review|check|freeze|evidence|converge|graph|run|implement|archive> <spec-name> [T###]
skills: spec-to-prod
---
...
- `review <spec>` → reviewer agent in contract mode; at closing, feed it the complete implementation diff for implementation-diff mode. Read-only BLOCK/WARN/NIT findings
- `check <spec>` → `scripts/check.py specs/<spec>` and triage the output
- `freeze <spec>` → after an explicit user yes only: `scripts/freeze.py
  specs/<spec> --record` writes the approval hash manifest
- `evidence <spec>` → `scripts/audit.py evidence specs/<spec>` verifies
  freeze, lifecycle SHAs, and durable closing evidence (read-only)
...
- no verb → treat `$ARGUMENTS` as a feature request (a bug report is the `bugfix` verb); follow the skill's right-sized tiers (tiny work keeps artifacts/gates but may stay inline; large/high-risk work gets full waves, NFR triage, and both reviews)
